...
interface ILikeRepository {
    ...
    suspend fun observeTypeLikes(
        type: LikeType,
        userId: String?
    ): Flow<List<CachedLike?>> // Observes Room cache
...
class LikeRepository @Inject constructor(
    ...
    override suspend fun toggleLike(
        ...
        val likeDocId = when (type) {
            ...
            LikeType.COMMENT -> {
                if (postId == null) return@withContext ResultWrapper.Error(
                    IllegalArgumentException(
                        "postId needed for Comment"
                    )
                )
        ...
        try {
            ...
            Log.d(
                "LikeRepository",
                "Optimistic cache update for $likeDocId: isLiked=$intendedActionIsLike"
            )
        } catch (cacheError: Exception) {
            Log.e(
                "LikeRepository",
                "Error updating Room cache optimistically for $likeDocId",
                cacheError
            )
        ...
        try {
            firestore.runTransaction { transaction ->
                ...
                val paths = getFirestorePaths(type, targetId, likeDocId)
                ...
                if (intendedActionIsLike) { // User wants to LIKE
                    if (!likeExistsBackend) { // Only proceed if not already liked on backend
                        val likeObject =
                            createLikeObject(type, likeDocId, effectiveUserId, targetId, postId)
                        ...
                        Log.w(
                            "LikeRepository",
                            "[TX] Like intended for $likeDocId, but already liked on backend. No op."
                        )
                    }
                } else { // User wants to UNLIKE
                    if (likeExistsBackend) {
                        ...
                        Log.w(
                            "LikeRepository",
                            "[TX] Unlike intended for $likeDocId, but not liked on backend. No op."
                        )
                    }
                }
            }.await()
            ...
            try {
                ...
                Log.e(
                    "LikeRepository",
                    "Error removing pending state from cache for $likeDocId after successful TX",
                    cacheError
                )
            }
            ...
            Log.e(
                "LikeRepository",
                "Firestore transaction failed for $likeDocId. Cache remains pending.",
                e
            )
    ...
    private fun getFirestorePaths(
        ...
        return when (type) {
            ...
            LikeType.COMMENT -> FirestoreLikePaths(
                ...
                metricsDocRef = firestore.collection(Comment.COMMENTS_COLLECTION)
                    .document(targetId),
