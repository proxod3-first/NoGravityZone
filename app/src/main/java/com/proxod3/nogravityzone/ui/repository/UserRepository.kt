...
class UserRepository @Inject constructor(
            ...
            try {
                ...
                if (snapshot.exists()) {
                    ...
                    if (userDetails != null) {
                        ...
                        Log.e(
                            "UserRepository",
                            "Failed to convert Firestore document ${snapshot.id} to User object."
                        )
                    ...
                    if (snapshot != null) {
                        val posts = snapshot.documents.mapNotNull { doc ->
                            try {
                                ...
                                Log.e(
                                    "UserRepository",
                                    "Failed converting post snapshot ${doc.id}",
                                    e
                                )
                                null
                            }
    ...
    override suspend fun updateUserProfileImage(imagePath: String): ResultWrapper<String> {
        ...
        val userImageStorageRef = storage.reference
            .child(USER_IMAGES)
            .child(uid + "_" + imagePath.substringAfterLast("/")) // Include UID for uniqueness
        ...
        return try {
            ...
            try {
                ...
                try {
                    Log.w(
                        "UserRepository",
                        "Firestore update failed for profile pic, attempting to delete Storage file."
                    )
                    userImageStorageRef.delete().await()
                } catch (deleteError: Exception) {
                    Log.e(
                        "UserRepository",
                        "Failed to delete profile pic from Storage after DB error",
                        deleteError
                    )
                }
